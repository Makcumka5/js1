...
const config = {
    ...
    isObstaclesOn() {
        return this.settings.withObstacles;
    },
...
const snake = {
    ...
    getNextStepHeadPoint() {
        ...
        switch (this.direction) {
            case "up":
                if (headPoint.y - 1 < 0)
                    return { x: headPoint.x, y: config.getRowsCount() - 1 };
                return { x: headPoint.x, y: headPoint.y - 1 };
            case "right":
                if (headPoint.x + 1 === config.getColsCount())
                    return { x: 0, y: headPoint.y };
                return { x: headPoint.x + 1, y: headPoint.y };
            case "down":
                if (headPoint.y + 1 === config.getRowsCount())
                    return { x: headPoint.x, y: 0 };
                return { x: headPoint.x, y: headPoint.y + 1 };
            case "left":
                if (headPoint.x - 1 < 0)
                    return { x: config.getColsCount() - 1, y: headPoint.y };
                return { x: headPoint.x - 1, y: headPoint.y };
        }
    },
...
const game = {
    ...
    addScores() {
        ...
        scoresSpan.innerText = this.scores;
    },
    resetScores() {
        this.scores = 0;
        const scoresSpan = document.getElementById("scores");
